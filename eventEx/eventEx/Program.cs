...
namespace eventEx
{
    class Program
    {
        ...
        public class DownloadStartEventArgs  //事件参数类
        {
        ...
        public class DownloadEndEventArgs    //事件参数类
        {
            public string Url { get { return _url; } set { _url = value; } }
            private string _url;
        ...
        public class DownloadingEventArgs     //事件参数类
        {
            public string Url { get { return _url; } set { _url = value; } }
            private string _url;
        ...
        public class Crawler
        {
            ...
            public void Craw()
            {
                while (true)
                {
                    ...
                    {
                        DownloadStart(this, new DownloadStartEventArgs(url));  //触发/调用事件，事件参数就是委托规定的参数
                    }
                    ...
                    for (long i = 0; i < size + 1024; i += 1024)
                    {
                        ...
                        {
                            Downloading(this, new DownloadingEventArgs(url, percent));  //触发/调用事件
                        }
                    }

                    if (DownloadEnd != null) //下载结束的事件发生
                    {
                        DownloadEnd(this, new DownloadEndEventArgs(url, size));  //触发/调用事件
                    }
                }
        ...
        class Test
        {
            ...
            static void ShowStart(object sender, DownloadStartEventArgs e)   //事件发生时，要调用的函数，事件处理者
            {
